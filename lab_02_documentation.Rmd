...
date: "Reading for Lab on Monday Sept.\ 3."
...
x = summarize(nashville_weather, prcp.max = max(prcp), tmin.min = min(tmin), 
              tmax.max = max(tmax))
x
```
...
annual_extremes = weather %>% mutate(year = year(date)) %>%
  ...
  ungroup()                                   # that in, then if any year has a
                                              # missing value for even one day,
                                              # the tmax or tmin for that year
                                              # will be recorded as NA (missing)
...
ggplot(data = annual_extremes # the data
  ...
  geom_line(aes(x = year, y = tmin, group = location), color = "dark blue", 
            size = 1) +
  geom_line(aes(x = year, y = tmax, group = location), color = "dark red", 
            size = 0.3) +
  geom_point(aes(x = year, y = tmin, color = location, shape = "min"), 
             size = 2) +
  geom_point(aes(x = year, y = tmax, color = location, shape = "max"), 
...
ggplot(data = annual_extremes_gathered # the data
  ...
  geom_point(aes(x = year, y = value, color = location, shape = Temperature), 
             size = 2) +
  scale_size_manual(values = c(tmax = 0.5, tmin = 0.1)) + # set coordinates 
                                                          # for "size"
  scale_x_continuous(limits=c(1990,2000), 
                     breaks = c(1990, 1992, 1994, 1996, 1998, 2000)) + 
  # ^^^ Set coordinates for the x-axis.
  # Below: one of many options for setting the color palette: the Brewer
  # palettes are very good for people with color-blindness.
